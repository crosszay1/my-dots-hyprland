...
TERMINAL_RESTORE_SCRIPT="$RESTORE_SCRIPT_DIR/__restore_terminal_wallpaper.sh"
...
kill_existing_terminal_wallpaper() {
    # Kill by window class using hyprctl if available
    if command -v hyprctl &>/dev/null; then
        local pids
        pids=$(hyprctl clients -j 2>/dev/null | jq -r '.[] | select(.class == "wallpaper-terminal") | .pid')
        if [[ -n "$pids" ]]; then
    ...
    # Fallback: kill by matching command pattern
    pkill -f "wallpaper-terminal" 2>/dev/null || true
}

create_terminal_restore_script() {
    ...
    if command -v kitty &>/dev/null; then
        terminal_bin="kitty"
        terminal_opts="--class wallpaper-terminal --override background_opacity=0.0 --override window_padding_width=0 --override confirm_os_window_close=0 -1"
    elif command -v foot &>/dev/null; then
        terminal_bin="foot"
        terminal_opts="--app-id wallpaper-terminal --override=colors.background=00000000 --override=pad=0x0"
    else
        echo "No supported terminal (kitty, foot) found for terminal wallpaper" >&2
        return 1
    fi
    ...
    {
        printf '#!/bin/bash\n'
        printf '# Generated by switchwall.sh - Do not modify manually.\n'
        printf '# Time: %s\n\n' "$(date)"
        declare -f kill_existing_terminal_wallpaper
        printf '\nkill_existing_terminal_wallpaper\n\n'
        printf 'nohup %s %s -e bash -c %q >/dev/null 2>&1 &\n' \
            "$terminal_bin" "$terminal_opts" "while true; do $terminal_cmd; sleep 1; done"
    } > "$TERMINAL_RESTORE_SCRIPT.tmp"

    mv "$TERMINAL_RESTORE_SCRIPT.tmp" "$TERMINAL_RESTORE_SCRIPT"
    chmod +x "$TERMINAL_RESTORE_SCRIPT"
}

remove_terminal_restore() {
    mkdir -p "$RESTORE_SCRIPT_DIR"
    cat > "$TERMINAL_RESTORE_SCRIPT.tmp" << EOF
#!/bin/bash
# The content of this script will be generated by switchwall.sh - Don't modify it by yourself.
EOF
    mv "$TERMINAL_RESTORE_SCRIPT.tmp" "$TERMINAL_RESTORE_SCRIPT"
...
set_terminal_command() {
    local cmd="$1"
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        jq --arg cmd "$cmd" '.background.terminalCommand = $cmd' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
    fi
}
...
switch() {
    ...
    # When switching to image/video, clear any active terminal command wallpaper
    kill_existing_terminal_wallpaper
    remove_terminal_restore
    set_terminal_command ""
...
main() {
    ...
    if [[ -z "$imgpath" && -z "$color_flag" && -z "$noswitch_flag" && -z "$terminal_command_flag" ]]; then
    ...
    # Handle terminal command wallpaper mode
    if [[ -n "$terminal_command_flag" ]]; then
        kill_existing_terminal_wallpaper
        kill_existing_mpvpaper
        if [[ -z "$terminal_command" ]]; then
            # Clear terminal command wallpaper
            set_terminal_command ""
            remove_terminal_restore
        else
            set_terminal_command "$terminal_command"
            if create_terminal_restore_script "$terminal_command"; then
                nohup bash "$TERMINAL_RESTORE_SCRIPT" >/dev/null 2>&1 &
            fi
        fi
        return 0
    fi
